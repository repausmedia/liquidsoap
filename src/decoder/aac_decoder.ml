...
module Make (Generator : Generator.S_Asio) = struct
  let create_decoder input =
    ...
    let input, drop, pos = buffered_input input aacbuf offset (len - offset) in
    ...
    in
    {
      ...
      decode =
        (fun gen ->
          ...
          if len = aacbuflen then begin
            let pos, data =
              Faad.decode dec (Bytes.unsafe_of_string aacbuf) 0 len
            in
            begin
              try processed := !processed + Array.length data.(0) with _ -> ()
            end;
            drop pos;
...
module Make_mp4 (Generator : Generator.S_Asio) = struct
  ...
  let create_decoder input =
    ...
    let decode gen =
      ...
      incr sample;
      begin
        try pos := !pos + Array.length data.(0) with _ -> ()
      end;
      let content = resampler ~audio_src_rate:(float sample_freq) data in
      Generator.set_mode gen `Audio;
      Generator.put_audio gen content 0 (Array.length content.(0))
