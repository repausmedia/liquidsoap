    ...
    Some
      ( name,
        fun () ->
          try f ()
          with exn ->
            log#severe "Decoder %S betrayed us on %S! Error: %s" name filename
...
module Buffered (Generator : Generator.S) = struct
  let make_file_decoder ~filename ~close ~kind ~remaining decoder gen =
    ...
    let fill frame =
      ...
      if not !decoding_done then begin
        try
      ...
      if not (c_end = frame_size && Frame.type_has_kind c_type kind) then begin
    ...
    let fseek len =
      ...
      if len < 0 || len > gen_len then begin
        Generator.clear gen;
